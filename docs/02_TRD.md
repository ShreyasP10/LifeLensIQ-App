...
```
users/{uid}                                    — profile
users/{uid}/timetable/{docId}                  — one doc per day (array of slots)
...
### 7.1 Foreground service `LifeiqTrackerService`
...
- [ ] Add Android app (package `com.lifeiq.app`), download `google-services.json` → `app/`.
