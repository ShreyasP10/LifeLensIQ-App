...
### 2.2 Collection: `users/{uid}/timetable/{day}` — one doc per day
...
Room stores `payloadJson` (serialized payload); Firestore stores structured `payload` map. Exporters read either.
